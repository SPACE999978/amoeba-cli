...
impl LabApp {
    ...
    pub(super) fn cache_help_page(&mut self, page_id: String, page: GitbookPage) {
        ...
        self.help_page_cache_order
            .retain(|cached| cached != &page_id);
        self.help_page_cache_order.push_back(page_id.clone());
        self.help_pages.insert(page_id, page);
        self.help_page_revision = self.help_page_revision.wrapping_add(1);
        self.help_render_cache.get_mut().take();

        while self.help_page_cache_order.len() > HELP_PAGE_CACHE_LIMIT {
            let removable = self
                .help_page_cache_order
                .iter()
                .position(|cached| cached != &self.help_selected_page_id)
                .unwrap_or_default();
            if let Some(expired) = self.help_page_cache_order.remove(removable) {
                self.help_pages.remove(&expired);
            }
        }
    }
