...
impl LabApp {
    ...
    pub(super) fn guide_available_targets(&self) -> Vec<guide::GuideTargetSnapshot> {
        ...
        if self.trade_ticket.is_some() {
            ...
            targets.push(guide_ui_target(
                ...
                control.1,
                "final_control",
                "Only the user may activate this local review control; the TUI never prepares, signs, or sends the order.",
                "focus",
                true,
            ));
        }
                ...
                actions.push(guide::GuideFormActionSnapshot {
                    ...
                    fields: vec![
                        guide::GuideFieldSnapshot {
                            field_id: "price".to_string(),
                            label: action.price_label().to_string(),
                            value: price,
                            required: true,
                            editable_by_guide: true,
                        },
                        guide::GuideFieldSnapshot {
            ...
            for action in WriterAction::ALL {
                ...
                actions.push(guide::GuideFormActionSnapshot {
                    ...
                    fields: form
                        .fields
                        .iter()
                        .map(guide_writer_field_snapshot)
                        .collect(),
    ...
    pub(super) fn guide_active_form(&self) -> Option<guide::GuideFormSnapshot> {
        if let Some(ticket) = self.trade_ticket.as_ref() {
            return Some(guide::GuideFormSnapshot {
                ...
                purpose: "Reviewable fixed-risk order values for a local CLI preparation template. The Guide cannot review the final ticket, and the TUI never prepares, signs, or sends the order."
                    .to_string(),
                fields: vec![
                    guide::GuideFieldSnapshot {
                        field_id: "price".to_string(),
                        label: ticket.action.price_label().to_string(),
                        value: (!ticket.premium_input.is_empty()).then(|| ticket.premium_input.clone()),
                        required: true,
                        editable_by_guide: ticket.confirmation.is_none() && !ticket.submitting,
                    },
                    guide::GuideFieldSnapshot {
                        field_id: "quantity".to_string(),
                        label: "Contracts".to_string(),
                        value: (!ticket.quantity_input.is_empty()).then(|| ticket.quantity_input.clone()),
                        required: true,
                        editable_by_guide: ticket.confirmation.is_none() && !ticket.submitting,
                    },
                ],
        ...
        if let Some(form) = self.writer_form.as_ref() {
            ...
            return Some(guide::GuideFormSnapshot {
                ...
                fields: form
                    .fields
                    .iter()
                    .map(|field| guide::GuideFieldSnapshot {
                        field_id: field.key.to_string(),
                        label: field.label.to_string(),
                        value: (!field.secret && !field.value.is_empty())
    ...
    pub(super) fn guide_visible_cues(&self) -> Vec<String> {
        ...
        match self.screen {
            ...
            LabScreen::Chain => {
                ...
                cues.push(
                    "A staged ticket is editable only; Enter validates the local risk preview and shows a direct CLI preparation template. The current TUI ticket never prepares, signs, or sends."
                        .to_string(),
                );
            }
            LabScreen::Chart => {
    ...
    pub(super) fn apply_guide_stage_trade(
        ...
        self.status = "Ticket staged. Check price, contracts, max loss, and payout; press Enter yourself to validate the local preview and show the CLI preparation template. The TUI will not prepare, sign, or send."
            .to_string();
        Ok(())
    }
    ...
    pub(super) fn apply_guide_fill_active_form(
        ...
        if target_id == "form:trade-ticket" {
            ...
            self.guide.focused_control = Some("control:trade:review".to_string());
            self.status = "Ticket fields staged. Press Enter yourself to validate the local risk preview and show the CLI preparation template; the TUI will not prepare, sign, or send."
                .to_string();
            return Ok(());
        }
    ...
    pub(super) fn apply_guide_focus_control(&mut self, target_id: &str) -> Result<(), String> {
        ...
        match target_id {
            "control:trade:review" => {
                ...
                self.status = "Review / export is highlighted. Press Enter yourself to validate the local risk preview and show the CLI preparation template; the TUI will not prepare, sign, or send."
                    .to_string();
            }
            "control:trade:confirm" => {
            ...
            "control:mcp:connection" => {
                ...
                self.status = "The connection button is highlighted. Only your click changes assistant settings. Petri MCP never signs or submits transactions."
                    .to_string();
            }
            "control:update" => {
...
fn guide_liquidity_preview_action_value(action: liquidity::LiquidityPreviewAction) -> &'static str {
    match action {
        liquidity::LiquidityPreviewAction::Add => "add",
        liquidity::LiquidityPreviewAction::Remove => "remove",
        liquidity::LiquidityPreviewAction::ClosePosition => "close-position",
    }
}
...
fn guide_liquidity_field_snapshots(
    ...
    vec![
        snapshot(
            ...
            guide_liquidity_preview_action_value(form.action).to_string(),
        ),
...
fn guide_writer_field_snapshot(field: &ActionFormField) -> guide::GuideFieldSnapshot {
    guide::GuideFieldSnapshot {
        field_id: field.key.to_string(),
        label: field.label.to_string(),
        value: (!field.secret && !field.value.is_empty()).then(|| field.value.clone()),
        required: field.required,
        editable_by_guide: !field.secret,
    }
}
