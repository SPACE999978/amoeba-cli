...
impl LabApp {
    ...
    pub(super) fn request_settlement(
        ...
        if !refresh
            && self.settlement_bundle.as_ref().is_some_and(|bundle| {
                bundle.market_id == market_id && bundle.expiry_id == expiry_id
            })
        {
            self.loading_settlement = false;
    ...
    pub(super) fn apply_trade_submit_result_at(
        ...
        let waiting = !ok && message.contains("Waiting for current finalized state");
        ...
        self.status = if waiting {
            format!(
                "{} order is waiting for current finalized state. Try again shortly.",
                action.label()
            )
        } else if ok {
            format!("{} order submitted.", action.label())
        } else if details_in_ticket {
            format!(
                "{} order failed. Review the ticket and try again.",
                action.label()
            )
        } else {
            format!(
                "{} order failed. Check Activity before trying again.",
                action.label()
            )
        };
        ...
        if waiting {
            result_modal.failure_reason = Some(
                "Current finalized state is not available yet; no transaction target was prepared."
                    .to_string(),
            );
