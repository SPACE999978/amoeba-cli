...
impl LabApp {
    ...
    pub(super) fn clickable_button_at(&self, cli: &Cli, root: Rect, column: u16, row: u16) -> bool {
        ...
        if !self.trade_submit_is_running()
            && let Some(order_area) = active_order_ticket_area(body_layout.selected_area, self)
        {
    ...
    pub(super) fn handle_mouse_click(
        ...
        if let Some(order_area) = active_order_ticket_area(body_layout.selected_area, self) {
        ...
        if let Some(focus) = chain_focus_at(body_layout.selected_area, column, row) {
            self.set_focus(focus);
            match focus {
